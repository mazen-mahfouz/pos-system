    ...
    <div class="px-[10px]">
      ...
      <section class="h-[calc(100vh-238px)] lg:h-[calc(100vh-295px)] xl:h-[calc(100vh-310px)] pb-[10px] overflow-auto grid content-start gap-[5px] mt-[10px] transition-all duration-300 ease-in-out">
        ...
        <TransitionGroup
          ...
          tag="div"
          class="grid gap-[10px]"
          :class="{'grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 2xl:grid-cols-8': OrderStore.$state.openOrder, 'grid-cols-7 lg:grid-cols-6 xl:grid-cols-13 2xl:grid-cols-12': !OrderStore.$state.openOrder }"
        >
          <div
            v-for="item in items"
            :key="item.name"
